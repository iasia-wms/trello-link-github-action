...
try {
  ...
  getBoard(boardIdentifier, apiKey, token)
    .then((board) => {
      if (!board || !board.boardId) {
        throw new Error("board could not be resolved");
      }
      return Promise.all(
        distinctMatches.map((match) => {
          return getCard(match.id, board, apiKey, token).then((card) => {
            if (card) {
              console.log(
                `attaching '${match.title}' (${match.url}) to card '${card.cardName}' (${card.cardNumber})`
              );
...
function getMarkerRegex(marker) {
  ...
  if (/^[a-zA-Z]+-$/g.test(marker)) {
    ...
    throw new Error(
      "marker must be an alpha string, an alpha string followed by '-', or one of !@#$%^&*+="
    );
  }

  const regex = new RegExp(`${marker}[a-zA-Z0-9]+\\b`, "g");
  return regex;
}
...
function fromCommit(regex, payload, markerLength) {
  ...
  const commits =
    ((payload && payload.commits && payload.commits.length) || 0) > 0
      ? payload.commits
      : [payload.head_commit];
      ...
      .then((resp) => {
        ...
        return (
          ...
          json
            .filter(
              (b) =>
                b.name === boardIdentifier ||
                b.shortLink === boardIdentifier ||
...
function getCard(cardIdentifier, board, apiKey, token) {
  console.log(
    `searching trello board '${board.boardName}' for card '${cardIdentifier}'`
  );
  return axios
    .get(
    ...
    .catch((err) => {
      console.log(
        `Did not find card '${cardIdentifier}' on board '${board.boardName}'`,
        err
      );
    });
}
...
function attachUrl(cardDetails, url, title, apiKey, token) {
  return axios
    ...
    .then(() => {
      console.log(
        `attached '${title}' (${url}) to card '${cardDetails.cardName}' (${cardDetails.cardNumber})`
      );
    });
}
